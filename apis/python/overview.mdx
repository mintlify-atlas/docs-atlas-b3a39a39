...
<Tabs>
  <Tab title="Table API">
    ...
    # Source: generate random word IDs at 5 rows/second
    words = ["flink", "python", "streaming", "state", "checkpoint"]

    t_env.create_temporary_table(
        "source",
        t_env.from_elements([(w,) for w in words * 10], schema=["word"])
        .__class__  # placeholder — use TableDescriptor in production
    )
    ...
    table = t_env.from_elements([(w,) for w in words * 10], schema=["word"])
    result = (
        table
